...
type TabType = 'vibe' | 'ceremonies' | 'settings'
...
export function TeamDetailContent({ team, vibeMetrics, vibeInsights = [], ceremoniesSessions = [] }: TeamDetailContentProps) {
  ...
  const getInitialTab = (): TabType => {
    ...
    const validTabs = ['vibe', 'ceremonies', 'settings']
    if (urlTab && validTabs.includes(urlTab)) {
      if (urlTab === 'settings') return urlTab
  ...
  useEffect(() => {
    ...
    const validTabs = ['vibe', 'ceremonies', 'settings']
    if (urlTab && validTabs.includes(urlTab)) {
      if (urlTab === 'settings' || team.tools_enabled.includes(urlTab as 'vibe' | 'ceremonies')) {
        setActiveTab(urlTab)
      }
    }
